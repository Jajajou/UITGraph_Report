...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/img/system_architecture.png}
    \caption{Tổng quan kiến trúc hệ thống UITRaph và các lớp xử lý}
...
\begin{enumerate}
    ...
    \item \textbf{Knowledge Graph (Neo4j/NetworkX):} Phục vụ suy luận đa bước và truy vấn cấu trúc.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/img/indexing_flow.png}
...
\begin{itemize}
    \item \textbf{Neo4j/NetworkX:} Lưu trữ đồ thị tri thức, biểu diễn các quan hệ cấu trúc như prerequisites (tiên quyết), belongs\_to (thuộc về).
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/img/lightrag_architecture.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/img/metadata_rag_subgraph.png}
...
\textbf{Bước 4 - Tính điểm tin cậy (Confidence Scoring):}

\begin{equation}
confidence = 0.4 \times completeness\_score + 0.4 \times llm\_confidence + 0.2 \times chunk\_quality
\end{equation}

Trong đó:
\begin{itemize}
    \item $completeness\_score$: Số trường metadata được trích xuất thành công / Tổng số trường
    \item $llm\_confidence$: Điểm tin cậy từ LLM extraction
    \item $chunk\_quality$: Điểm relevance trung bình từ Cross-Encoder
\end{itemize}

Phân loại confidence:
\begin{itemize}
    \item 0.9-1.0 (Excellent): Sử dụng trực tiếp kết quả RAG (hệ thống đạt 0.92)
    \item 0.7-0.9 (Good): Sử dụng với cảnh báo
    \item 0.5-0.7 (Fair): Kết hợp với Regex validation
    \item $< 0.5$ (Low): Fallback hoàn toàn sang Regex
\end{itemize}
...
\begin{table}[H]
\centering
\caption{So sánh hiệu quả các phương pháp trích xuất metadata}
...
Trong kiến trúc tích hợp với LightRAG, quá trình nạp và xử lý tài liệu diễn ra theo cơ chế bất đồng bộ (asynchronous processing). Khi nội dung được gửi đi, hệ thống chỉ nhận lại ngay lập tức một mã theo dõi (\texttt{track\_id}) thay vì định danh tài liệu (\texttt{doc\_id}) chính thức.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/img/query_pipeline.png}
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.95\textwidth]{images/img/deployment_architecture.png}
...
\subsection{Embedding Model và Reranker}
\label{subsec:embeddingmodel}

\textbf{Embedding Model:}
\begin{itemize}
    \item Model: AlTeamVN/Vietnamese\_Embedding\_v2
    \item Dimension: 1024-dim
    \item Tối ưu: Embedding vector được tinh chỉnh đặc biệt cho văn bản tiếng Việt chuyên ngành
    \item Use case: Sử dụng cho cả indexing (Metadata RAG) và query retrieval
\end{itemize}

\textbf{Reranker:}
\begin{itemize}
    \item Model: namdp-ptit/ViRanker
    \item Architecture: Cross-encoder dựa trên PhoBERT
    \item Performance: 85\%+ accuracy trên văn bản giáo dục tiếng Việt
    \item Use case: Two-stage retrieval trong Metadata RAG Subgraph
\end{itemize}
